...
def trainMultiple(inputData, desiredOutputData, weights):
    ...
    for inputs in inputData:
        weightAdjustments[i] = train(inputData[i], desiredOutputData[i], weights)
        i = i + 1
    transposedWeightAdjustments = np.transpose(weightAdjustments)
    meanWeightAdjustment = np.array([np.mean(transposedWeightAdjustments[0]),
                                     np.mean(transposedWeightAdjustments[1]),
                                     np.mean(transposedWeightAdjustments[2])])
